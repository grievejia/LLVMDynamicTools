...
static MyInterpreter *TheInterpreter;
...
static ExFunc lookupFunction(const Function *F) {
	...
	if (FnPtr == 0)  // Try calling a generic function... if it exists...
		FnPtr = (ExFunc)(intptr_t)sys::DynamicLibrary::SearchForAddressOfSymbol("lle_X_" + F->getName().str());
	if (FnPtr != 0)
...
static bool ffiInvoke(RawFunc Fn, Function *F, const std::vector<GenericValue> &ArgVals, const DataLayout *TD, GenericValue &Result) {
	ffi_cif cif;
	...
	if (ArgVals.size() > NumArgs && F->isVarArg()) {
		report_fatal_error("Calling external var arg function '" + F->getName() + "' is not supported by the Interpreter.");
	}

	unsigned ArgBytes = 0;

	std::vector<ffi_type*> args(NumArgs);
	for (Function::const_arg_iterator A = F->arg_begin(), E = F->arg_end(); A != E; ++A) {
...
GenericValue MyInterpreter::callExternalFunction(Function *F, const std::vector<GenericValue> &ArgVals)
{
	TheInterpreter = this;
	...
	if (ExFunc Fn = (FI == ExportedFunctions->end()) ? lookupFunction(F) : FI->second) {
	...
	if (F->getName() == "__main")
		errs() << "Tried to execute an unknown external function: " << *F->getType() << " __main\n";
	else
		report_fatal_error("Tried to execute an unknown external function: " + F->getName());
	return GenericValue();
}
...
static GenericValue lle_X_sprintf(FunctionType *FT, const std::vector<GenericValue> &Args) {
	...
	case '%': {                    // Handle format specifiers
		...
		*FB++ = *FmtStr++;
		char Last = *FB++ = *FmtStr++;
		unsigned HowLong = 0;
		while (Last != 'c' && Last != 'd' && Last != 'i' && Last != 'u' && Last != 'o' && Last != 'x' && Last != 'X' && Last != 'e' && Last != 'E' && Last != 'g' && Last != 'G' && Last != 'f' && Last != 'p' && Last != 's' && Last != '%') {
		...
		*FB = 0;
		...
		case 'x': case 'X':
			if (HowLong >= 1) {
				if (HowLong == 1 &&	TheInterpreter->getDataLayout()->getPointerSizeInBits() == 64 &&
...
static GenericValue lle_X_sscanf(FunctionType *FT, const std::vector<GenericValue> &args) {
	...
	GenericValue GV;
	GV.IntVal = APInt(32, sscanf(Args[0], Args[1], Args[2], Args[3], Args[4], Args[5], Args[6], Args[7], Args[8], Args[9]));
	return GV;
}
...
static GenericValue lle_X_scanf(FunctionType *FT, const std::vector<GenericValue> &args) {
	...
	GenericValue GV;
	GV.IntVal = APInt(32, scanf( Args[0], Args[1], Args[2], Args[3], Args[4], Args[5], Args[6], Args[7], Args[8], Args[9]));
	return GV;
}
...
void MyInterpreter::initializeExternalFunctions() {
	sys::ScopedLock Writer(*FunctionsLock);
